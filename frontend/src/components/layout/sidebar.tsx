...
import { UtensilsCrossed, ClipboardList, Star, Settings, Package, Users, BarChart3, LogOut } from "lucide-react"
...
export default function Sidebar({ user }: SidebarProps) {
  ...
  const adminMenuItems = [
    ...
    { href: "/home/analytics", label: "Análisis de Rendimiento", icon: BarChart3 },
    { href: "/home/settings", label: "Configuración", icon: Settings },
  ]
  ...
  return (
    <aside className="w-20 bg-primary-dark flex flex-col items-center py-6 space-y-8 border-r border-border">
      ...
      <Link href="/home" className="text-2xl font-bold text-white hover:opacity-80 transition-smooth">
          ...
          return (
            <Link
              ...
              className={`w-12 h-12 flex items-center justify-center rounded-lg transition-smooth ${
                pathname === item.href ? "bg-primary text-white" : "text-white/60 hover:text-white hover:bg-primary/20"
              }`}
              title={item.label}
            >
      ...
      <button className="w-12 h-12 flex items-center justify-center rounded-lg text-white/60 hover:text-white hover:bg-error/20 transition-smooth">
        <LogOut size={24} />
      </button>
    </aside>
  )
